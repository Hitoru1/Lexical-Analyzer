        ...
        while self.current_char != None:
            ...
            if self.current_char in WHITESPACE:
                ...
                if self.current_char == '\n':
                    ...
                    self.advance()
                    tokens.append(Token(WHITESPACE_SPACE, 'WHITESPACE', pos_start, self.pos.copy()))
                    continue
                elif self.current_char == '\t':
                    self.advance()
                    tokens.append(Token(WHITESPACE_TAB, '\\t', pos_start, self.pos.copy()))
                    continue
            ...
            ############### COMMENTS - BASED ON PAPER (~ for single, ~~ for multi) ###############
                ...
                if self.current_char == '~':
                    ...
                    while self.current_char != None:
                        ...
                        self.advance()
                    tokens.append(Token(COMMENT_MULTI, comment_val.strip(), pos_start, self.pos.copy()))
                    continue
                else:
                    # Single-line comment
                    comment_val = ''
                    while self.current_char != None and self.current_char != '\n':
                        comment_val += self.current_char
                        self.advance()
                    tokens.append(Token(COMMENT_SINGLE, comment_val.strip(), pos_start, self.pos.copy()))
                    continue
            ...
            ############### KEYWORDS OR IDENTIFIERS ###############
                ...
                if id_str in keywords:
                    ...
                    if id_str in ['Yes', 'No']:
                        tokens.append(Token(LIT_BOOLEAN, id_str, pos_start, pos_end))
                    else:
                        tokens.append(Token(token_map[id_str], id_str, pos_start, pos_end))
                else:
                    ...
                    # Valid identifier
                    tokens.append(Token(IDENTIFIER, id_str, pos_start, pos_end))
                continue
            ...
            ############### NUMBERS ###############
                ...
                # Integer literal
                if dot_count == 0:
                    tokens.append(Token(LIT_NUMBER, num_str, pos_start, pos_end))
                else:
                    tokens.append(Token(LIT_DECIMAL, num_str, pos_start, pos_end))
                continue
            ...
            ############### STRING LITERALS - DOUBLE QUOTES ###############
                ...
                pos_end = self.pos.copy()
                tokens.append(Token(LIT_STRING, string_val, pos_start, pos_end))
                continue
            ...
            ############### CHARACTER LITERALS - SINGLE QUOTES ###############
                ...
                pos_end = self.pos.copy()
                tokens.append(Token(LIT_CHARACTER, char_val, pos_start, pos_end))
                continue
            ...
            ############### OPERATORS ###############
                ...
                if self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_ADDITION_ASSIGN, '+=', pos_start, self.pos.copy()))
                elif self.current_char == '+':
                    self.advance()
                    tokens.append(Token(OP_INCREMENT, '++', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_ADDITION, '+', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char and self.current_char in DIGITS:
                    ...
                    if len(tokens) == 0 or tokens[-1].type in [
                        ...
                        # Create token
                        if dot_count == 0:
                            tokens.append(Token(LIT_NUMBER, num_str, num_start, num_end))
                        else:
                            tokens.append(Token(LIT_DECIMAL, num_str, num_start, num_end))
                        continue
                
                # Not a negative number, treat as subtraction operator
                if self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_SUBTRACTION_ASSIGN, '-=', pos_start, self.pos.copy()))
                elif self.current_char == '-':
                    self.advance()
                    tokens.append(Token(OP_DECREMENT, '--', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_SUBTRACTION, '-', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char == '*':
                    self.advance()
                    tokens.append(Token(OP_EXPONENTIATION, '**', pos_start, self.pos.copy()))
                elif self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_MULTIPLICATION_ASSIGN, '*=', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_MULTIPLICATION, '*', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_DIVISION_ASSIGN, '/=', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_DIVISION, '/', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_MODULUS_ASSIGN, '%=', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_MODULUS, '%', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_EQUAL_TO, '==', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_ASSIGNMENT, '=', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_NOT_EQUAL, '!=', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_LOGICAL_NOT, '!', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_GREATER_EQUAL, '>=', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_GREATER_THAN, '>', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char == '=':
                    self.advance()
                    tokens.append(Token(OP_LESS_EQUAL, '<=', pos_start, self.pos.copy()))
                else:
                    tokens.append(Token(OP_LESS_THAN, '<', pos_start, self.pos.copy()))
                continue
                ...
                if self.current_char == '&':
                    self.advance()
                    tokens.append(Token(OP_LOGICAL_AND, '&&', pos_start, self.pos.copy()))
                else:
                ...
                if self.current_char == '|':
                    self.advance()
                    tokens.append(Token(OP_LOGICAL_OR, '||', pos_start, self.pos.copy()))
                else:
                    errors.append(LexicalError(pos_start, self.pos.copy(), 
                        'Invalid character "|" (did you mean "||"?)'))
                continue
            ...
            ############### DELIMITERS ###############
                ...
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_LEFT_PAREN, '(', pos_start, self.pos.copy()))
                continue

            elif self.current_char == ')':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_RIGHT_PAREN, ')', pos_start, self.pos.copy()))
                continue

            elif self.current_char == '[':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_LEFT_BRACKET, '[', pos_start, self.pos.copy()))
                continue

            elif self.current_char == ']':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_RIGHT_BRACKET, ']', pos_start, self.pos.copy()))
                continue

            elif self.current_char == '{':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_LEFT_BRACE, '{', pos_start, self.pos.copy()))
                continue

            elif self.current_char == '}':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_RIGHT_BRACE, '}', pos_start, self.pos.copy()))
                continue

            elif self.current_char == ';':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_SEMICOLON, ';', pos_start, self.pos.copy()))
                continue

            elif self.current_char == ':':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_COLON, ':', pos_start, self.pos.copy()))
                continue

            elif self.current_char == ',':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_COMMA, ',', pos_start, self.pos.copy()))
                continue

            elif self.current_char == '.':
                pos_start = self.pos.copy()
                self.advance()
                tokens.append(Token(DELIM_DOT, '.', pos_start, self.pos.copy()))
                continue
...
class KuCodeLexerGUI:
    def __init__(self, root):
        ...
        terminal_frame = tk.Frame(root, bg="white", relief=tk.RAISED, bd=1, height=150)
...
finish"""
    ...
    def analyze(self):
        ...
        if not source.strip():
            self.terminal_text.insert(tk.END, "Error: No source code to analyze\n")
            return
        ...
        if errors:
            self.terminal_text.insert(tk.END, "✗ Lexical analysis completed with errors:\n\n", "error")
            for error in errors:
                self.terminal_text.insert(tk.END, str(error) + "\n", "error")
            self.terminal_text.tag_config("error", foreground="#ff6b6b")
        else:
            self.terminal_text.insert(tk.END, "✓ Lexically correct - no lexical errors detected.\n", "success")
    ...
    def save_results(self):
        ...
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                ...
                f.write("TOKENS\n")
                f.write("=" * 80 + "\n")
                f.write(f"{'Type':<30} {'Lexeme':<30} {'Line':<10} {'Col':<10}\n")
                f.write("-" * 80 + "\n")
                
                for item in self.token_table.get_children():
                    values = self.token_table.item(item)['values']
                    f.write(f"{values[0]:<30} {values[1]:<30} {values[2]:<10} {values[3]:<10}\n")
